...
export function createSqliteImportActions(options) {
    const { db } = options;
    const importBackup = (payload) => {
        ...
        const run = db.transaction(() => {
         ...
         ON CONFLICT(key) DO UPDATE SET
           ...
           updated_at = CURRENT_TIMESTAMP`).run("orgName", normalized.data.orgName);
            db.prepare(`INSERT INTO settings (key, value, updated_at)
         VALUES (?, ?, CURRENT_TIMESTAMP)
         ON CONFLICT(key) DO UPDATE SET
           value = excluded.value,
           updated_at = CURRENT_TIMESTAMP`).run("social", JSON.stringify(normalized.data.social));
      ...
      `);
            for (const employee of normalized.data.employees) {
                insertEmployee.run(employee.id, employee.name, employee.idCard, companyIds.get(employee.companyShort) ?? null, employee.dept, employee.position, employee.type, employee.baseSalary, employee.subsidy, employee.hasSocial ? 1 : 0, employee.hasLocalPension ? 1 : 0, employee.fundAmount);
            }
...
//# sourceMappingURL=sqlite-import.js.map