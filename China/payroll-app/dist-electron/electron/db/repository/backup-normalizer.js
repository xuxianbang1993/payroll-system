import { DEFAULT_ORG_NAME, normalizeCompanies, normalizeEmployees, normalizeSocialConfig, } from "./defaults.js";
...
function ensureCompanyCoverage(companies, employees) {
    ...
    for (const employee of employees) {
        ...
        if (!existing.has(employee.companyShort)) {
            existing.set(employee.companyShort, resolveCompanyFullName(employee.companyShort, employee.company));
        }
    }
    return [...existing.entries()].map(([short, full]) => ({ short, full }));
}
function harmonizeEmployeesWithCompanies(employees, companies) {
    const byShort = new Map(companies.map((company) => [company.short, company.full]));
    return employees.map((employee) => {
        if (employee.companyShort === "") {
            return employee;
        }
        const full = byShort.get(employee.companyShort) ??
...
export function normalizeBackupPayload(value) {
    const record = asRecord(value) ?? {};
    const hasExtendedData = Boolean(asRecord(record.data));
    const sourceFormat = hasExtendedData
        ? "extended"
        : "legacy";
...
//# sourceMappingURL=backup-normalizer.js.map